(function(window, document, red5pro, SubscriberBase) {
  ...
  var mungeBox = document.getElementById('sdp-munge-box');
  var mungeSubmit = document.getElementById('sdp-munge-submit');
  var mungeCancel = document.getElementById('sdp-munge-cancel');
  disableMunge();

  function enableMunge () {
    mungeSubmit.disabled = mungeCancel.disabled = false;
  }
  function disableMunge () {
    mungeSubmit.disabled = mungeCancel.disabled = true;
  }
  ...
  function onSubscriberEvent (event) {
    ...
    if (event.type === red5pro.RTCSubscriberEventTypes.ANSWER_SUSPENDED_FOR_SDP_MUNGE) {
      var responder = event.data;
      var sdp = responder.getOriginalSDP();
      mungeBox.value = sdp;
      enableMunge();
      mungeSubmit.addEventListener('click', function() {
        responder.submit(mungeBox.value);
        disableMunge();
      });
      mungeCancel.addEventListener('click', function() {
        responder.cancel();
        disableMunge();
      });
    }
